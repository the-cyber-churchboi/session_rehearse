...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        $query = "SELECT sender_id, COUNT(*) AS notread_count
        FROM admin_messages
        ...
        $stmt->execute();
        $notreadCounts = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Prepare an array to store the counts for each sender
        $countsBySender = [];
        foreach ($notreadCounts as $count) {
            $countsBySender[$count['sender_id']] = $count['notread_count'];
        }
