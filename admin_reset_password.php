...
function validateResetToken($token) {
    global $pdo;
    $sql = "SELECT email, token_expiration FROM admin_registration WHERE reset_token = :token";
...
function resetPasswordWithToken($token, $password) {
    ...
    $sql = "UPDATE admin_registration SET password = :password, reset_token = NULL, token_expiration = NULL WHERE reset_token = :token";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
