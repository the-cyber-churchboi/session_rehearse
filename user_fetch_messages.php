...
try {
    ...
    $updateSql = "UPDATE messages SET message_status = 'read' WHERE receiver_id = :userId";
    $updateStmt = $pdo->prepare($updateSql);
    $updateStmt->bindParam(":userId", $userId, PDO::PARAM_INT);
    $updateStmt->execute();

    $sqlAdmin = "SELECT messages.*, admin_registration.full_name AS sender_name, admin_registration.profession AS admin_profession, messages.file_path
          FROM messages
          INNER JOIN admin_registration ON messages.sender_id = admin_registration.unique_identifier
          WHERE messages.sender_id = :uniqueIdentifier AND messages.receiver_id = :userId
    ...
    // Modify your SQL query for user messages to include the status column
    $sqlUser = "SELECT messages.*, users.username AS sender_name, messages.file_path
             FROM messages
             INNER JOIN users ON messages.sender_id = users.unique_identifier
             WHERE messages.sender_id = :userId AND messages.receiver_id = :uniqueIdentifier
             ORDER BY messages.timestamp ASC";

    $stmtStudent = $pdo->prepare($sqlUser);
    ...
    while ($row = $stmtStudent->fetch(PDO::FETCH_ASSOC)) {
        $message = [
            ...
            "timestamp" => $row["timestamp"],
            "file_path" => $row["file_path"],
            "status" => $row["message_status"], // Include the status column
        ];
